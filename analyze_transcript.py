...
def system_snapshot():
    ...
    snap["thread_handoff_head"] = read_text(ROOT / "THREAD_HANDOFF_V4.6_2026-09-10.md")[:6000]   # 另一討論串的交接（V4.6 規格、否決清單、持倉）
    claims = read_text(CLAIMS)
    snap["existing_claims_tail"] = claims[-6000:]
    snap["rules"] = {
        "V4.6 進場（現行，9/10 上線）": "創2y月線ATH + 多頭排列 + 動能≥80 + 科技7族群 + 市值≥100億 + 0050>MA200 + 0050>自身20MA(V4.4減速器) + 訊號日未達注意股累積漲幅(V4.5) → 推播當天收盤前買（13:25 限價或盤後定價），絕不開盤買/市價單；每檔15萬",
        "V4.6 出場": "收盤跌破 20MA 或 成交價×0.93 先到者、或自進場後高點回落30% → 隔天開盤競價賣；盤中跌破不算",
        "已被回測否決（聽到同類論點直接標 ❌，不再排回測）": "拉回20MA才買、漲到警示線出場、0050距年線>25%就暫停、外資籌碼評分、訊號日漲停不買、開盤後等回落再買、限價掛整天、隔兩天再買、觸發當天收盤停損、盤中觸價停損、龍頭補漲/落後補漲；共同點都是「提早動作求安心」砍掉右尾",
...
SYSTEM_PROMPT = """你是一個台股量化投資系統的「論點驗證庫」管理員。系統有兩個引擎：短線動能 V4.6（創高當天收盤前追價、20MA/-7% 出場；V4.4 減速器與 V4.5 警示濾網內含）與長線 3 年翻倍池（thesis-based）。
