# -*- coding: utf-8 -*-
"""
⚠️ DEPRECATED — 用戶決定移除「漲停續航」過濾，v2 邏輯歸於 v1
   保留檔案供後續參考實作細節，不再執行回測
...
決策原因：
  - 過嚴 → 第一次跑 0 筆交易
  - 即使放寬，本質上與 v1 不衝突，續航是「觀察指標」不是「過濾條件」
  - 真實熱錢可能初次漲停就進場，不需等 2 次累積
═════════════════════════════════════════════════
"""
import sys
print("⚠️ backtest_hot_money_v2.py 已棄用，請用 backtest_hot_money_radar.py")
sys.exit(0)
